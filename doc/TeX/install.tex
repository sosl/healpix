...
\newcommand{\healpixmail}{{\em healpix-support} {\tt at} {\em lists.sourceforge.net}}
...
\subsubsection{IDL/GDL configuration}
...
sourced as described in \S~\ref{subsub:conf}, the aliases {\tt hidl}, {\tt hidlde}, {\tt hgdl} and {\tt hgdlde}
...
\item Other input fiducial $C(\ell)$, all in $(\mu$K$)^2$ and defined on the multipole range $[0, \ell_{\rm max}]$, have been included for convenience in 
{\tt Healpix\_\hpxversion/test/} in a \healpix compatible format.
\\
\begin{tabular}{l l l l} 
\hline 
  \textbf{File name} & \textbf{alias} & \textbf{Origin} & $\ell_{\rm max}$\\ \hline
...
Starting with \healpix 3.12, {\tt pkg-config} ({\tt .pc}) files are generated
...
If the 
\htmladdnormallink{{\tt pkg-config} software}{http://en.wikipedia.org/wiki/Pkg-config} 
...
the \healpix {\tt pkg-config} files above are known to it (either by moving/copying them
...
{\tt ifort 14.0.1.106}}) \hfill \\
     ...
     \item {\bf G95} compiler available for Linux, Mac OSX, Windows, Sun and HP platforms with 32 and 64 bit architectures (eg, x86 and x86-64). In the latter case, the '32bit default integer' (32bit DI) version of {\tt g95} {\em must} be used.
         \hfill \\ \htmladdnormallink{{\tt http://www.g95.org}}{http://www.g95.org}
    \end{itemize}
...
\item Go to \htmladdnormallink{{\tt https://www.cygwin.com/}}{https://www.cygwin.com/}
...
\item
  ...
  \htmladdnormallink{{\tt ftp://ftp.gtlib.gatech.edu}}{ftp://ftp.gtlib.gatech.edu}).
\item
...
\end{itemize}
\item with Xquartz (default under Moutain Lion (10.8.*), Mavericks (10.9.*) and Yosemite (10.10.*), available
for download for El Capitan (10.11.*) and Sierra (10.12.*)):\\
{{\tt defaults write org.macosforge.xquartz.X11 wm\_click\_through -bool true}}
\item with MacPort's X11 (package xorg-server):\\
{{\tt defaults write org.macports.X11 wm\_click\_through -bool true}}
\end{itemize}
at your X11 prompt and restart X11.\\
Note that the command {\tt ls -lrt  \$HOME/Library/Preferences/*[xX]11*plist} 
can be used to determine the X window system installed on
your Mac.
See also \htmladdnormallink{{\tt http://www.idlcoyote.com/misc\_tips/maccursor.html}}
{http://www.idlcoyote.com/misc_tips/maccursor.html} and
\htmlref{{\tt mollcursor}}{idl:mollcursor} documentation in \linklatexhtml{''IDL
...
If \healpix 3.30 is used instead, {\tt Healpix\_3.30/src/idl/visu/proj2out.pro} must be replaced with the one found in 
\htmladdnormallink{%
...
% \item \healpix 3.30 must be installed, and {\tt Healpix\_3.30/src/idl/visu/proj2out.pro} must be replaced with the one found in 
% \htmladdnormallink{%
% http://sourceforge.net/p/healpix\-/code\-/HEAD\-/tree\-/branches\-/branch\_v330r807\-/src\-/idl\-/visu\-/proj2out.pro}{%
% http://sourceforge.net/p/healpix/code/HEAD/tree/branches/branch_v330r807/src/idl/visu/proj2out.pro}.
...
\htmladdnormallink{\tt plplot}{http://plplot.sourceforge.net/source/index.html}, 
\htmladdnormallink{\tt gsl}{http://www.gnu.org/software/gsl}, 
\htmladdnormallink{\tt readline}{http://ftp.gnu.org/pub/gnu/readline/} 
and 
\htmladdnormallink{\tt zlib}{http://www.zlib.net/}) 
GDL must also have been (pre-)compiled with links to
\begin{itemize}
\item \htmladdnormallink{\tt ImageMagick}{http://www.imagemagick.org}
(or \htmladdnormallink{\tt GraphicsMagick}{http://www.graphicsmagick.org/}) 
to produce PNG and JPEG output files, 
\item \htmladdnormallink{\tt pslib}{http://pslib.sourceforge.net} to produce PostScript and PDF files (in the latter case,
a recent version of \htmladdnormallink{\tt ghostscript}{http://www.ghostscript.com/}, i.e. 9.07 or more, is also recommended).
\end{itemize}
...
\item When run under GDL 0.9.6, and if the requirements stated above are met, 
the visualization routines
\htmlref{{\tt azeqview}, {\tt cartview}, {\tt gnomview}, {\tt mollview} and {\tt orthview}}{idl:mollview} 
will produce correct screen (X) outputs and PS, PDF, PNG and JPEG images, with the following caveats:
\begin{itemize}
\item they won't produce \mylinkext{idl:mollview:gif}{GIF} files (because GDL's {\tt write\_gif} is not yet functional); 
\item the \mylinkext{idl:mollview:transparent}{\tt transparent} keyword will be ignored in the
production of \mylinkext{idl:mollview:png}{PNG} files under GDL (except when \mylinkext{idl:mollview:truecolors}{\tt TrueColors} is set and $N$x3 map is provided), 
and for the same reason, the \htmlref{\tt hpx2gs}{idl:hpx2gs} routine won't mark missing pixels as
transparent in the output PNG file;
\item  and finally, the \mylinkext{idl:mollview:pfonts}{\tt pfonts} keyword will not allow the selection of other fonts than Hershey vectorial fonts (pfonts[0]=-1).
% \item and finally, the virtual window (\mylinkext{idl:mollview:window}{\tt window}$<0$, used for remote production of PNG images over slow network connections) won't work as expected, except in cases where
% \mylinkext{idl:mollview:shaded}{\tt shaded} or 
% \mylinkext{idl:mollview:truecolors}{\tt TrueColors} is set.
\end{itemize}
All other features work properly, including the \mylinkext{idl:mollview:latex}{\tt Latex} keyword.
...
\subsection{Bug corrections and Improvements in Version \hpxversion}%(2016-?)
...
\subsubsection[General]{General}
	...
	\item Interactive {\tt configure} script now supports MINGW environment (for Windows), 
		and better detects {\tt gcc} and {\tt python} versions;
...
\subsubsection[C++]{\htmladdnormallink{C++}{http://healpix.sourceforge.net/html/index_cxx.htm}}
	...
	(see \htmladdnormallink{\tt http://sourceforge.net/p/healpix/bugs/72}{http://sourceforge.net/p/healpix/bugs/72})
	\end{itemize}
\subsubsection[Fortran]{%
 	Fortran 90 \linklatexhtml{facilities}{facilities.pdf}{facilities.htm} and
 	\linklatexhtml{subroutines}{subroutines.pdf}{subroutines.htm}}
...
\subsubsection[Python]{\htmladdnormallink{Python}{http://healpy.readthedocs.org/en/latest/}}
	\begin{itemize}
	\item Switch to 
	\htmladdnormallink{{\tt healpy 1.9.1}}{https://github.com/healpy/healpy/releases} 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%----------------------------------------------------
\section{Appendix II: Older changes (versions 3.00 to 3.30)}
...
\item moved from {\tt alice2} to {\tt alice3}, which produces FITS \healpix maps as output.
 These can be visualized more flexibly with external tools.
\item switch from custom {\tt xcomplex} class to {\tt std::complex};
\item {\tt rangeset} class has been redesigned.
\end{itemize}
...
\begin{itemize}
 \item addition of \htmlref{{\tt ialteralm}}{{idl:ialteralm}} to modify
  Spherical Harmonics coefficients ($a_{lm}$).
%
